...
export class CSharpTokenizer {
  ...
  private currentLineTokens: Token[] = [];
  ...
  private handleNumber(): boolean {
    ...
    // Verificar sufijos válidos (f, d, m, l)
    if (this.position < this.source.length) {
      const nextChar = this.source[this.position].toLowerCase();
      const validSuffixes = ['f', 'd', 'm', 'l'];
  ...
  private handleOperator(): void {
    ...
    // Verificar si es operador de 2 caracteres
    if (this.position < this.source.length) {
      const twoCharOp = value + this.source[this.position];
...
private getMatchingClose(openChar: string): string {
  ...
  private getTokenCategory(tokenType: TokenType): string {
    switch (tokenType) {
      case TokenType.KEYWORD: return 'Palabras Reservadas';
      case TokenType.IDENTIFIER: return 'Variables';
      case TokenType.NUMBER: return 'Números';
      ...
      case TokenType.GROUPING_OPEN:
        return this.tokens[this.tokens.length - 1]?.value === '{' ? 'Llaves Abiertas' : 'Signos de Agrupación';
      case TokenType.GROUPING_CLOSE:
        return this.tokens[this.tokens.length - 1]?.value === '}' ? 'Llaves Cerradas' : 'Signos de Agrupación';
